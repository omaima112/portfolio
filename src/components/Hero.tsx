...
export function Hero({ onNavigate }: HeroProps) {
  ...
  const nameEn = 'Fatima M. Mushtaq';
  const nameAr = 'فاطمة محمد مشتاق';
  return (
  <section className={`relative overflow-hidden bg-gradient-to-br from-[#F6E1F0] via-[#EED1E6] to-white min-h-screen flex items-center py-20 md:py-32${isArabic ? ' rtl' : ''}`}> 
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row items-center gap-12 md:gap-16">
          {/* Profile Image (always left) */}
          <div className="relative group order-1 md:order-1 flex-shrink-0" style={{minWidth:'16rem'}}>
          ...
          {/* Hero Content (always right, right-aligned) */}
          <div className="flex-1 text-right md:text-right order-2 md:order-2 flex flex-col items-end justify-center">
            {/* Degree info moved to footer to avoid duplicate; kept visually minimal here */}
            
            <h1
              className="hero-name text-[#5A2653] mb-2 font-extrabold leading-tight text-right"
              style={{ fontSize: 'min(6vh, 4rem)', lineHeight: 1, fontFamily: isArabic ? 'Cairo, Noto Sans Arabic, Arial, sans-serif' : undefined, fontWeight: isArabic ? 900 : undefined }}
            >
            ...
            <p
              className="font-body italic text-right mb-6 leading-9 text-2xl md:text-3xl"
              dir={isArabic ? 'rtl' : 'ltr'}
              style={{ color: '#A86A9A', fontFamily: isArabic ? 'Cairo, Noto Sans Arabic, Arial, sans-serif' : undefined, fontWeight: isArabic ? 700 : undefined }}
            >
              {t('hero.quote')}
            </p>

            <p className="font-body text-gray-700 mb-8 max-w-2xl text-lg text-right">
              {t('hero.bio')}
            </p>

            <div className="flex flex-col sm:flex-row gap-4 justify-end md:justify-end">
              <button
